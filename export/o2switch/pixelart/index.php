...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; border-radius: 0 !important; }
        ...
        /* Par défaut (Desktop) : on cache la section mobile */
        ...
        @media (max-width: 768px) {
            ...
            #canvasMobileSection .section-content { 
                flex: 1; 
                display: flex; 
                flex-direction: column; 
            }
            ...
            #projectName { width: 120px !important; }
            .bg-bottom-panel {
                flex-direction: column;
                align-items: stretch;
                padding: 6px 10px;
...
<body>
    ...
    <header>
        ...
        <div class="header-actions">
            <input type="text" id="projectName" value="Mon Super Projet" style="width: 150px;" oninput="triggerAutoSave()">
    ...
    <script>
        ...
        function updateSVFromEvent(e) {
            ...
            x = Math.max(0, Math.min(x, svCanvas.width));
            y = Math.max(0, Math.min(y, svCanvas.height));
        ...
        function setCurrentColor(col) {
            ...
            if (typeof col === 'object' && col.empty) {
                ...
                preview.innerHTML = '?';
                document.getElementById('inputR').value = 0;
        ...
        function renderPalette() {
            ...
            customPalette.forEach((item, index) => {
                ...
                swatch.oncontextmenu = (e) => {
                    ...
                    if (customPalette.length > 1) {
                        ...
                        if (activeSwatchIndex >= customPalette.length) {
                            activeSwatchIndex = customPalette.length - 1;
                        }
                        setCurrentColor(customPalette[activeSwatchIndex]);
                        triggerAutoSave();
                    } else {
                        alert("La palette doit contenir au moins une case.");
                    }
                };
            ...
            addBtn.innerHTML = '+';
            addBtn.title = 'Créer une nouvelle case vide';
            addBtn.onclick = (e) => {
                e.preventDefault();
        ...
        function initGrid(savedPixels = null) {
            ...
            canvasMobile.width = canvas.width;
            canvasMobile.height = canvas.height;
            bgCanvasMobile.width = canvas.width;
            bgCanvasMobile.height = canvas.height;

            if (savedPixels && savedPixels.length === gridSize) {
                gridData = savedPixels;
            } else {
                gridData = [];
        ...
        function redraw() {
            [ctx, ctxMobile].forEach(cContext => {
                ...
                cContext.strokeStyle = 'rgba(255, 255, 255, 0.05)';
                cContext.lineWidth = 0.5;
                for (let i = 0; i <= gridSize; i++) {
                    cContext.beginPath();
                    cContext.moveTo(i * pixelSize, 0);
                    cContext.lineTo(i * pixelSize, canvas.height);
                    cContext.stroke();

                    cContext.beginPath();
                    cContext.moveTo(0, i * pixelSize);
                    cContext.lineTo(canvas.width, i * pixelSize);
                    cContext.stroke();
                }
            });
        }
        ...
        function handleAction(e) {
            ...
            if (typeof currentItem === 'object' && currentItem.empty && currentTool !== 'eraser' && currentTool !== 'picker') {
                return;
            }

            if (currentTool === 'pencil') {
                gridData[pos.y][pos.x] = currentColor;
                redraw();
                triggerAutoSave();
            } else if (currentTool === 'eraser') {
                gridData[pos.y][pos.x] = null;
                redraw();
                triggerAutoSave();
                ...
                if (gridData[pos.y][pos.x]) {
                    ...
                    if (!customPalette.includes(currentColor)) {
                        customPalette[activeSwatchIndex] = currentColor;
                        renderPalette();
                    }
                }
                setTool('pencil');
        ...
        function loadBackground(event) {
            ...
            reader.onload = function(e) {
                bgImg = new Image();
                bgImg.onload = function() { redrawBg(); };
                bgImg.src = e.target.result;
            };
            reader.readAsDataURL(file);
        }

        function loadBackgroundMobile(event) {
        ...
        function redrawBg() {
            [ {ctx: bgCtx, canvas: bgCanvas}, {ctx: bgCtxMobile, canvas: bgCanvasMobile} ].forEach(item => {
                ...
                if (bgImg && showBg) {
                    item.ctx.globalAlpha = bgOpacityVal;
                    item.ctx.drawImage(bgImg, 0, 0, item.canvas.width, item.canvas.height);
                    item.ctx.globalAlpha = 1.0;
                }
            });
        }
        ...
        function toggleBackground() {
            ...
            const btnMob = document.getElementById('toggleBgBtnMobile');
            if(btnMob) btnMob.innerText = btnText;
        ...
        function removeBackground() {
            ...
            const inputMob = document.getElementById('bgInputMobile');
        ...
        function updateOpacity(val) {
            ...
            const opMob = document.getElementById('opacityValMobile');
            if(opMob) opMob.innerText = pct;
            const rangeMob = document.getElementById('bgOpacityMobile');
            if(rangeMob) rangeMob.value = val;
            const rangeDesk = document.getElementById('bgOpacity');
            if(rangeDesk) rangeDesk.value = val;
            ...
            fetch('save.php')
                ...
                .then(data => {
                    ...
                    if (projectsList.length > 0) {
                        loadProject(projectsList[projectsList.length - 1].id);
                    } else {
                        newProject();
                    }
                });
        ...
        function renderProjectsList() {
            ...
            listEl.innerHTML = '';
            if (projectsList.length === 0) {
                listEl.innerHTML = '<span style="font-size:0.75rem; color:var(--text-muted);">Aucun projet enregistré</span>';
                return;
            }
        ...
        function triggerAutoSave() {
            ...
            statusEl.innerText = "Modifications...";
        ...
        function saveProject(asNew = false, isAuto = false) {
            ...
            const payload = {
                action: 'save',
                id: asNew ? null : currentProjectId,
                name: name,
                width: gridSize,
                height: gridSize,
                pixels: gridData,
            ...
            .then(data => {
                if (data.success) {
                    ...
                    fetch('save.php').then(res => res.json()).then(list => {
                        projectsList = list;
                        renderProjectsList();
                    });
                }
        ...
        function loadProject(id) {
            ...
            if (p.sectionsOrder && Array.isArray(p.sectionsOrder)) {
                const sidebar = document.getElementById('sidebarSections');
                p.sectionsOrder.forEach(secKey => {
                    const el = sidebar.querySelector(`[data-section="${secKey}"]`);
            ...
            setTimeout(() => { isInitialized = true; }, 400);
        }
        ...
        function newProject() {
            ...
            document.getElementById('projectName').value = "Nouveau Projet";
