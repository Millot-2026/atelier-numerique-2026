...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { font-family: 'Segoe UI', sans-serif; background: #f4f7f6; color: #2c3e50; margin: 0; padding: 20px; }
        h1 { text-align: center; color: #e67e22; font-size: 1.6rem; margin-bottom: 30px; }
        .journey { max-width: 800px; margin: 0 auto; display: flex; flex-direction: column; gap: 12px; }
        .step { display: flex; gap: 16px; background: white; border-radius: 10px; padding: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 5px solid #e67e22; }
...
if (isset($_POST['generate'])) {
    ...
    // Copie du JSON dans le dossier local pour autonomie
    ...
    // 3. Copie miroir globale vers _www/export/
    // dirname(__DIR__) = _www/ ✅ (user_generator.php est à la racine de user_journey-v1.0/)
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .container { width: 100%; max-width: 100%; margin: auto; background: #222; padding: 20px; border: 1px solid #333; border-radius: 8px; box-sizing: border-box; }
        h1 { font-size: 1.5rem; text-align: center; }
        
        /* FIX ULTIME : flex-wrap nowrap + min-width 0 sur le parent et les enfants */
        .row { display: flex; flex-wrap: nowrap; align-items: center; margin-bottom: 10px; background: #2a2a2a; padding: 8px; border-radius: 4px; gap: 8px; }
        .drag-handle { cursor: grab; padding: 5px; color: #666; font-size: 20px; user-select: none; flex-shrink: 0; }
        
        .input-group { display: flex; gap: 8px; flex-grow: 1; min-width: 0; }
        
        /* Force la compression des inputs */
        ...
        /* Le bouton rouge ne doit jamais bouger */
        .btn-remove { 
...
<div class="container">
    ...
    <div id="journey-preview" style="margin-top: 30px; padding: 20px; background: #222; border: 2px solid #333; border-radius: 8px;">
